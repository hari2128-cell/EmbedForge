...
function isImageFile(item: Material) { return /\.(?:avif|gif|jpe?g|png|webp)$/i.test(item.name) }
...
function MaterialViewer({ item, onClose, onPrevious, onNext, position, total, watermark }: { item: Material; onClose: () => void; onPrevious: () => void; onNext: () => void; position: number; total: number; watermark: string }) {
  ...
  return <section className="material-viewer protected-content" aria-label={`Learning material viewer: ${materialDisplayName(item)}`} onContextMenu={event => event.preventDefault()}><div className="material-viewer-bar"><button className="btn-secondary material-back" onClick={onClose}>← Back to Learning Material</button><div><p className="label-mono">SECURE LEARNING MATERIAL</p><h2>{materialDisplayName(item)}</h2><p className="material-context">{materialContext(item)}</p></div></div><div className="material-watermark" aria-hidden="true">Licensed access · {watermark}</div><div className={`material-viewer-frame ${isImageFile(item) ? 'is-image-material' : ''}`}>{loading ? <div className="material-loading"><p className="label-mono">LOADING DOCUMENT</p><p className="body-standard">Preparing protected content…</p></div> : error ? <div className="material-loading"><p style={{color: '#ff8585'}}>{error}</p><button className="btn-secondary" onClick={loadMarkdown}>Try again</button></div> : isMarkdownFile(item) ? <article className="markdown-content"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{a: ({href, children}) => <a href={href} target="_blank" rel="noreferrer noopener">{children}</a>}}>{markdown}</ReactMarkdown></article> : isImageFile(item) && item.url ? <div className="document-image-viewport"><img className="document-image" src={item.url} alt={`${materialDisplayName(item)} learning material`} draggable={false} /></div> : item.url ? <iframe title={item.name} src={item.url} className="document-frame" /> : <div className="material-loading"><p style={{color: '#ff8585'}}>Unable to open this document. Please try again.</p></div>}</div>{total > 1 && <nav className="material-navigation" aria-label="Material navigation"><button className="btn-secondary" onClick={onPrevious} disabled={position === 0}>← Previous</button><span className="label-mono">MATERIAL {position + 1} / {total}</span><button className="btn-secondary" onClick={onNext} disabled={position === total - 1}>Next →</button></nav>}</section>
}
